...
public class MinAndMaxWebTestGenerator {
    public static void main(String[] a) {
        ...
        try
        {
            writer = new BufferedWriter( new FileWriter( "/Users/Lucifer/Documents/Development/workspace/software_testing/src/pset6/MinAndMaxWebTestSuite.java"));
            writer.write(suite);
        }
        catch ( IOException e)
        {
    ...
    String testsuite() {
        ...
        sb.append("\t\twd.get(\"file:///Users/Lucifer/Documents/Development/workspace/software_testing/src/pset6/minandmax.html\");\n");
        sb.append("\t}\n\n");
        ...
        for(String i: valuePool){
        	for(String j : valuePool){
        		for(String k : valuePool){
        			for(String m : methodPool){
        				for(String c : clickPool){
        					sb.append("\t@Test public void t" + count + "() {\n");
        					sb.append("\t\twe = wd.findElement(By.id(\""+m+"\"));\n");
        					sb.append("\t\twe.click();\n");
        					...
        					if (i == "Four" || j == "Four" || k == "Four") {
        						...
        						if(c.length() != 0){
        							if(m == "min"){
            							...
            							String tmp = Integer.toString(Math.max(Math.max(Integer.parseInt(i), Integer.parseInt(j)), Math.max(Integer.parseInt(j), Integer.parseInt(k))));
                						sb.append("\t\tassertEquals(\"max("+ i + ", " + j + ", " + k + ") = " + tmp +  "\", output);\n");	
            						}
        						}
        					}
